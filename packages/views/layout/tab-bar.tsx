...
import { useTabStore, useActiveGroup, type Tab } from "@multica/core/tabs";
...
/** Desktop-only: open an issue-detail session as a dedicated Electron window. */
export type TabBarIssueWindow = {
  pathFromUrl: (url: string) => string | null;
  open: (path: string, title: string) => void;
};

const TabBarIssueWindowContext = createContext<TabBarIssueWindow | null>(null);
...
function SortableTabItem({
  ...
  const issueWindow = useContext(TabBarIssueWindowContext);
  const issueWindowPath = issueWindow?.pathFromUrl(tab.url) ?? null;
  ...
  const handleOpenAsWindow = () => {
    if (!issueWindow || !issueWindowPath) return;
    issueWindow.open(issueWindowPath, title);
  };
...
export function TabBar({
  ...
  issueWindow?: TabBarIssueWindow | null;
}) {
  ...
  return (
    <TabBarIssueWindowContext.Provider value={issueWindow}>
    ...
    </TabBarIssueWindowContext.Provider>
  );
}
