...
Web and desktop share **session tabs** (in-app document tabs, one mounted session per workspace group). The store lives in `@multica/core/tabs`; the strip lives in `@multica/views/layout`. Below the compact breakpoint (1024px) the strip is hidden and modifier-clicks are browser tabs. Do not call this "mobile" — that word is reserved for the Expo app, which does not host session tabs. See `docs/adr/0001-web-session-tabs.md`.
